    ...
    @section('content')
        <x-dashboard.page-header title="مزودو الخدمات" :addRoute="route('dashboard.service_owners.create')" />
        ...
        <!-- Search & Filters -->
        <div class="my-2">
            <x-dashboard.search-sort :route="route('dashboard.service_owners.index')" :showName="true">
                <div class="col-lg-3 col-md-3 mt-1">
                    <input type="text" name="phone_number" value="{{ request()->get('phone_number') }}"
                        placeholder="ابحث برقم الهاتف" class="form-control bg-white">
                </div>
                <div class="col-lg-3 col-md-3 mt-1">
                    <select name="creator_user_id" class="form-select bg-white">
                        <option value="">أنشئ من قبل ؟</option>
                        @foreach ($creators as $creator)
                            <option value="{{ $creator->id }}"
                                {{ request()->get('creator_user_id') == $creator->id ? 'selected' : '' }}>
                                {{ $creator->full_name }}
                            </option>
                        @endforeach
                    </select>
                            ...
                            <td>
                                <div class="">
                                    @if ($user->deleted_at === null)
                                        <a href="{{ route('dashboard.service_owners.edit', $user->id) }}"
                                            class="btn btn-md rounded font-sm edit-data" data-id="{{ $user->id }}" title="تعديل المعلومات">
                                            <i class="material-icons md-edit"></i>
                                        </a>

                                        <form class="d-inline delete-form"
                                            action="{{ route('dashboard.service_owners.destroy', $user->id) }}"
                                        ...
                                        @if ($user->account_status === 'active')
                                            <a href="#"
                                                ...
                                                data-url="{{ route('dashboard.service_owners.deactivate', $user->id) }}"
                                        ...
                                        @else
                                            <a href="#"
                                                ...
                                                data-url="{{ route('dashboard.service_owners.activate', $user->id) }}"
                                    ...
                                    @if ($user->deleted_at)
                                        <form class="d-inline restore-form"
                                            action="{{ route('dashboard.service_owners.restore', $user->id) }}"
                                            method="POST" data-id="{{ $user->id }}">
                                            @csrf
                                            <button type="button"
                                    ...
                                    @endif
                                    <a href="{{ route('dashboard.service_owners.show', $user->id) }}"
        ...
        <x-dashboard.pagination :paginator="$serviceOwners" />

        {{-- @include('dashboard.users.serviceOwners.create') --}}
