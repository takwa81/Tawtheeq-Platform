    ...
    @section('content')
        <x-dashboard.page-header title="مسؤولي إدخال البيانات" addModalId="dataEntryModal" />
        ...
        <!-- Search & Filters -->
        <div class="my-2">
            <x-dashboard.search-sort :route="route('dashboard.data_entries.index')" :showName="true">
                                ...
                                <div class="">
                                    @if ($user->deleted_at === null)
                                        <a href="javascript:void(0)" class="btn btn-md rounded font-sm edit-data"
                                            ...
                                            data-phone_number="{{ $user->phone_number }}" title="تعديل المعلومات">
                                            <i class="material-icons md-edit"></i>
                                        </a>

                                        <form class="d-inline delete-form"
                                            action="{{ route('dashboard.data_entries.destroy', $user->id) }}"
                                        ...
                                        @if ($user->account_status === 'active')
                                            <a href="#"
                                                ...
                                                data-url="{{ route('dashboard.data_entries.deactivate', $user->id) }}"
                                        ...
                                        @else
                                            <a href="#"
                                                ...
                                                data-url="{{ route('dashboard.data_entries.activate', $user->id) }}"
                                    ...
                                    @if ($user->deleted_at)
                                        <form class="d-inline restore-form"
                                            action="{{ route('dashboard.data_entries.restore', $user->id) }}"
                                            method="POST" data-id="{{ $user->id }}">
                                            @csrf
                                            <button type="button"
                                    ...
                                    @endif
                                    <a href="{{ route('dashboard.data_entries.show', $user->id) }}"
        ...
        <x-dashboard.pagination :paginator="$dataEntries" />

        @include('dashboard.users.dataEntries.create')
