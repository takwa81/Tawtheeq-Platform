$(document).ready(function () {
    ...
    const modalTitle = $("#countryModalLabel");
    ...
    function renderRow(country) {
        return `
            <tr id="row-${country.id}">
                ...
                <td>${country.governorates_count}</td>
                <td>
                    ...
                    <form class="d-inline delete-form" action="/dashboard/countries/${country.id}" method="POST" data-id="${country.id}">
                    ...
                    <a href="/dashboard/countries/${country.id}" class="btn btn-md bg-secondary rounded font-sm">
    ...
    form.on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                const newRow = renderRow(country);

                if (isEdit) {
                    $(`#row-${country.id}`).replaceWith(newRow);
    ...
    modal.on("hidden.bs.modal", function () {
        ...
        form.attr("action", "/dashboard/countries");

        modalTitle.text('إضافة بلد جديد');
